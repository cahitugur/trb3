void first()
{
   ...
   int tdcmap[4] = { 0xC001, 0xC002, 0xC004, 0xC102 };

   trb3->SetHistFilling(4);

   // TDC subevent header id
   trb3->SetHadaqTDCId(tdcmap[0] & 0xF000);
   ...
   for (int cnt=0;cnt<4;cnt++) {
      ...
      // verify prefix
      if ((tdcmap[0] & 0xF000) != (tdcmap[cnt] & 0xF000)) {
         fprintf(stderr, "!!!! Wrong prefix in TDC%d, do not match with TDC0  %X != %X\n", cnt, (tdcmap[cnt] & 0xF000), (tdcmap[0] & 0xF000));
         exit(5);
      }
      ...
      //    if (cnt==0 || cnt==1) {
      int channels[] = { 25, 26, 27, 28, 29, 30, 31, 32, 0 };
      tdc->CreateHistograms( channels );

      tdc->SetRefChannel(28, 26, 0xffff, 4000, -2.,  2., true);
      tdc->SetRefChannel(30, 26, 0xffff, 4000, -2.,  2., true);
      tdc->SetRefChannel(32, 26, 0xffff, 4000, -2.,  2., true);
      //tdc->SetRefChannel(5, 1, 0xffff, 10000, -10., 10., true);
      
      // this is example how to specify conditional print
      // tdc->EnableRefCondPrint(1, -20000., -10000., 100);
      ...
      tdc->LoadCalibration(Form("calibrations/32PinAddOn_%04x.cal", tdcmap[cnt]));

      // calculate and write static calibration at the end of the run
      //tdc->SetWriteCalibration(Form("calibrations/32PinAddOn_%04x.cal", tdcmap[cnt]));
