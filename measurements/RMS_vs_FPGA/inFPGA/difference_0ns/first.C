void first()
{
   ...
   int tdcmap[4] = { 0xC010, 0xC011, 0xC012, 0xC013 };
   ...
   for (int cnt=0;cnt<4;cnt++) {
      ...
      tdc->SetRefChannel(3, 1, 0xffff, 4000, -2.,  2., true);
      tdc->SetRefChannel(5, 1, 0xffff, 4000, -2.,  2., true);
      tdc->SetRefChannel(7, 1, 0xffff, 4000, -2.,  2., true);
      ...
      tdc->LoadCalibration(Form("calibrations/fpga3-4_%04x.cal", tdcmap[cnt]));

      // calculate and write static calibration at the end of the run
      //tdc->SetWriteCalibration(Form("calibrations/fpga3-4_%04x.cal", tdcmap[cnt]));
